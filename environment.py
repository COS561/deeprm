...
class Env:
    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
        ...
        # rnn stuff
        if self.pa.rnn:
            self.rnn = tf_dist_rnn_object.dist_rnn(copy.deepcopy(pa))
            self.rnn.restore()
        else:
            self.rnn = None
        ...
        pa.simu_len = pa.simu_len + 50# self.rnn.SEQ_LEN
        print("ori sim len " + str(ori_simu_len))
        print("pa sim len " + str(pa.simu_len))
        print("rnn seq len " + str(50))# str(self.rnn.SEQ_LEN))

        if nw_len_seqs is None or nw_size_seqs is None:
            # generate new work
            self.nw_len_seqs, self.nw_size_seqs = \
                jd.generate_sequence_work(pa, seed=43)
        ...
        self.len_seeds_for_rnn = self.nw_len_seqs[:, :50]#:self.rnn.SEQ_LEN]
        self.res_seeds_for_rnn = self.nw_size_seqs[:, :50, :]#:self.rnn.SEQ_LEN, :]

        self.nw_len_seqs = self.nw_len_seqs[:, 50:]#self.rnn.SEQ_LEN:]
        self.nw_size_seqs = self.nw_size_seqs[:, 50:, :]#self.rnn.SEQ_LEN:, :]

        pa.simu_len = ori_simu_len

        print "pa", pa.simu_len
    ...
    def get_new_job_from_forecast(self, seq_idx):
        ...
        if seq_idx >= len(self.rnn.forecast_nw_len_seqs):
            idx = 0

        else:
            idx = seq_idx

        new_job = Job(res_vec=self.rnn.forecast_nw_size_seqs[idx, :],
                      job_len=self.rnn.forecast_nw_len_seqs[idx],
                      job_id=len(self.job_record.record),
                      enter_time=self.curr_time)
        return new_job
    ...
    def step(self, a, repeat=False, return_raw_jobs=False, forecasting=False):
        ...
        if status == 'MoveOn':
            ...
            if not done:
                ...
                if self.seq_idx < self.pa.simu_len:  # otherwise, end of new job sequence, i.e. no new jobs
                    if forecasting:
                        new_job = self.get_new_job_from_forecast(self.seq_idx + self.rnn_offset)
                    else:
    ...
    def replace_backlog_from_rnn(self):
        jobs_to_replace = self.job_backlog.curr_size
        for i in range(self.job_backlog.curr_size):
            if jobs_to_replace > 0:
                if self.job_backlog.backlog[i] is not None:
                    self.job_backlog.backlog[i] = self.get_new_job_from_forecast(
                        self.seq_idx + self.rnn_offset)
                    self.rnn_offset += 1
                    jobs_to_replace -= 1
