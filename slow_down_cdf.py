...
def launch(pa, pg_resume=None, render=False, plot=False, repre='image', end='no_new_job'):
    ...
    for seq_idx in xrange(pa.num_ex):
        for traj_type in traj_types:
            ...
            for test_type in test_types:
                ...
                print "mean slowdown ", np.mean(jobs_slow_down[test_type, traj_type][seq_idx])
                print "work complete", work_complete[test_type, traj_type][seq_idx]
    ...
    if pa.experiment_name is not None:
        if pa.rnn:
            np.save(pa.experiment_name + "_test_types_rnn.npy", test_types)
            np.save(pa.experiment_name + "_job_slow_down_rnn.npy", jobs_slow_down)
            np.save(pa.experiment_name + "_work_complete_rnn.npy", work_complete)
            np.save(pa.experiment_name + "_rews_rnn.npy", all_discount_rews)
        else:
            np.save(pa.experiment_name + "_test_types.npy", test_types)
            np.save(pa.experiment_name + "_job_slow_down.npy", jobs_slow_down)
            np.save(pa.experiment_name + "_work_complete.npy", work_complete)
            np.save(pa.experiment_name + "_rews.npy", all_discount_rews)
    else:
        np.save("test_types.npy", test_types)
        np.save("job_slow_down.npy", jobs_slow_down)
        np.save("work_complete.npy", work_complete)
        np.save("rews.npy", all_discount_rews)
