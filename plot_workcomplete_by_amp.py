...
if rnn:
    work_rnn = [f for f in onlyfiles if ((parallel == f.startswith("parallel_")) and f.endswith("_work_complete_rnn.npy"))]

    work_rnn = zip([f.strip("_work_complete_rnn.npy").strip("parallel_") for f in work_rnn],
        [np.load(f).item() for f in work_rnn])
#else:
work = [f for f in onlyfiles if ((parallel == f.startswith("parallel_")) and f.endswith("_work_complete.npy"))]

work = zip([f.strip("_work_complete.npy").strip("parallel_") for f in work],
...
plt.figure()
if rnn:
    plt.plot(amps, np.minimum(reg, reg_rnn), amps, halluc, amps, halluc_rnn, linewidth=2)
